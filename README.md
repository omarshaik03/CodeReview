...
- 🔍 Parse Git history with rich metadata and patch context.
- 🧠 Compose configurable LangChain review chains that call your preferred LLM provider.
- 🗂️ Modular design with clearly separated ingestion, review, and orchestration layers.
- 🧪 Comes with a testable core so you can stub the LLM for fast unit tests.
...
CLI (Typer/Rich)
   │
   ├── Services.ReviewService orchestrates workflow
   │       ├── CommitIngest.GitRepository pulls commits & diffs via GitPython
   │       └── Review.LangChainReviewAgent formats prompts & calls LLM through LangChain
   │
   └── Outputs markdown or Rich console summary per commit
```
...
Enjoy building your AI reviewer! 🎉
